...
export default function GraphView({ data, onNodeClick }: GraphViewProps) {
  ...
  const getNodeColor = (node: any) => {
    if (node.labels?.includes('Person')) {
      return node.properties?.isFlagged ? '#ef4444' : '#3b82f6';
    }
    if (node.labels?.includes('Account')) return '#10b981';
    if (node.labels?.includes('Device')) return '#f59e0b';
    if (node.labels?.includes('IP')) return '#8b5cf6';
    return '#64748b';
  };

  const getLinkColor = (link: any) => {
    if (link.properties?.isFraudRing) return '#ef4444';
    return '#334155';
  };

  return (
    <div ref={containerRef} className="w-full h-full min-h-[500px] bg-[#0f172a] rounded-xl overflow-hidden relative">
      {data.nodes.length === 0 ? (
